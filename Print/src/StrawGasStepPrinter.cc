...
mu2e::StrawGasStepPrinter::Print(const mu2e::StrawGasStep& obj, int ind, std::ostream& os) {
  ...
  os 
    ...
    << " " << std::setw(10) << std::setprecision(3) << obj.momvec().mag()
    << " " << std::setw(10) << std::setprecision(3) << obj.startPosition().x()
    << " " << std::setw(10) << std::setprecision(3) << obj.startPosition().y()
    << " " << std::setw(10) << std::setprecision(3) << obj.startPosition().z()
    << std::endl;
...
mu2e::StrawGasStepPrinter::PrintListHeader(std::ostream& os) {
  if(verbose()<1) return;
  os << " ind SimPart StrwInd      pdgId    eDep     length     time   Momentum      X0         Y0        Z0\n";
